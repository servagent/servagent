...
- **Tool selection**: `SERVAGENT_TOOLS` controls which tools are exposed. Default is `execute_command,write_file,read_file,edit_file,upload_file` (essential tools including file operations for reliable file writing — avoids slow/fragile heredocs via execute_command). Set to `all` to expose all 18 tools. Invalid tool names are logged as warnings and ignored. In `server.py`, the config string is parsed into a `set[str]` and passed to `register_tools(mcp, enabled_tools=...)`
