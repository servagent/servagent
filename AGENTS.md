...
- `src/servagent/config.py` — Configuration via pydantic-settings, loaded from env vars / `.env`. `_find_dotenv()` resolves the `.env` path at import time: `/opt/servagent/.env` (production) → repo root (dev) → cwd (fallback), so CLI commands like `servagent status` read the correct file regardless of the user's working directory
