...
servagent apikey renew       # Regenerate API key (invalidates current)
servagent apikey renew --yes # Non-interactive
servagent apikey remove      # Comment out API key in .env
servagent apikey remove --yes # Non-interactive
servagent oauth setup        # Generate OAuth credentials (issuer URL auto-detected)
servagent oauth setup --issuer-url https://example.com/mcp  # Explicit issuer URL
...
- `src/servagent/tools.py` — All MCP tool definitions (execute_command, read_file, write_file, edit_file, service_action, tail_file). Each tool has `ToolAnnotations` (read-only/destructive/idempotent hints) and docstrings that explicitly state it operates on the **remote** server to help AI clients differentiate from local operations. Exports `ALL_TOOL_NAMES` (set of all 6 tool names) and `register_tools(mcp, enabled_tools)` which conditionally registers only the requested tools
- `src/servagent/config.py` — Configuration via pydantic-settings, loaded from env vars / `.env`. `_find_dotenv()` resolves the `.env` path at import time: `/opt/servagent/.env` (production) → repo root (dev) → cwd (fallback), so CLI commands like `servagent status` read the correct file regardless of the user's working directory. The module-level `settings` object is a `_LazySettings` proxy that defers `Settings()` instantiation until first attribute access, avoiding `PermissionError` crashes when `.env` exists but is not readable (e.g. `servagent status` run without sudo). Key fields include `base_url` (public URL set by install.sh, e.g. `https://mcp.example.com`), `oauth_issuer_url`, `oauth_client_id`, `oauth_client_secret`, and TLS/host/port settings
...
- **Dual auth**: Two authentication mechanisms coexist. Bearer token (`API_KEY`) gives direct access to all endpoints. OAuth protects `/mcp` via access tokens from the SDK. Auth matrix: `/.well-known/` → always public (OAuth discovery); `/authorize`, `/token`, `/register`, `/revoke` → public when OAuth is active (307 redirects to `/mcp/…`); `/mcp/*` → delegated to SDK when OAuth is active, otherwise Bearer `API_KEY`; `/sse`, `/messages/`, `/upload` → Bearer `API_KEY`
- Auth middleware (`AuthMiddleware`, pure ASGI) runs on all routes. `.well-known` paths always pass through without auth (OAuth/resource discovery). Root-level OAuth paths (`/authorize`, `/token`, `/register`, `/revoke`) pass through when OAuth is active — they are 307 redirects to `/mcp/…`. When OAuth is active, all `/mcp/*` requests are delegated entirely to the SDK (the SDK's `BearerAuthBackend` validates tokens via `ServagentOAuthProvider.load_access_token()`, which also recognises a raw `API_KEY`). When OAuth is *not* active, `/mcp` falls through to the Bearer `API_KEY` check like other routes
...
- **OAuth endpoint redirects**: Some MCP clients (e.g. Claude.ai) send OAuth requests to root-level paths (`/authorize`, `/token`, `/register`, `/revoke`) instead of the sub-app paths (`/mcp/authorize`, etc.). When OAuth is active, four additional routes redirect these with 307 to `/mcp/…`, preserving query parameters. The auth middleware lets these paths through without requiring a Bearer token
- **Startup validation**: `main()` validates that `OAUTH_CLIENT_ID` and `OAUTH_CLIENT_SECRET` are either both set or both empty (fatal error). Also fatal if credentials are set but `OAUTH_ISSUER_URL` is empty — prevents a silently broken server that rejects all OAuth requests with "Invalid or missing API key"
...
- `install.sh` supports `--full-access` flag to grant sudo NOPASSWD to the service user and disable `NoNewPrivileges` in systemd; without the flag, an interactive prompt asks during installation. When a domain is provided, `install.sh` persists `SERVAGENT_BASE_URL=https://<domain>` in `.env`
...
- **Tool selection**: `SERVAGENT_TOOLS` controls which tools are exposed. Default is `execute_command,read_file,write_file,edit_file` (essential tools including file operations for reliable file writing — avoids slow/fragile heredocs via execute_command). Set to `all` to expose all 6 tools (adds service_action and tail_file). Invalid tool names are logged as warnings and ignored. In `server.py`, the config string is parsed into a `set[str]` and passed to `register_tools(mcp, enabled_tools=...)`
- **CLI**: The `servagent` entry point is `servagent.cli:cli` (click group). Without subcommand, it calls `server.main()` to start the server (backward-compatible). Both `cli` (no subcommand) and `run` call `_ensure_env_readable()` first — if the `.env` file exists but is not readable, the command re-executes itself with `sudo` automatically (prevents confusing `PermissionError` from pydantic-settings). `status` queries systemd via `systemctl show` and displays config from `settings` (API key masked to last 6 chars, OAuth issuer/client ID/secret shown with masking); it auto-escalates with sudo if the `.env` file is not readable (checks `os.access(R_OK)` before any output). `uninstall`/`update` delegate to shell scripts found via `_find_script()` (production: `/opt/servagent/`, dev: repo root), executed via `subprocess.run(["sudo", "bash", ...])`. CLI flags map directly to script flags. `apikey` is a click subgroup with three commands: `setup` (generates a `token_urlsafe(48)` API key, writes to `.env`; refuses if already configured), `renew` (regenerates key; requires `--yes` or interactive confirmation), `remove` (comments out `SERVAGENT_API_KEY` in `.env`; requires `--yes` or interactive confirmation). `oauth` is a click subgroup with three commands: `setup` (generates CLIENT_ID/CLIENT_SECRET, writes to `.env` with `OAUTH_ISSUER_URL`; auto-detects issuer URL via `_guess_issuer_url()` — falls back to `--issuer-url` if detection fails; refuses if OAuth already configured), `renew` (regenerates credentials, deletes OAuth database to invalidate all sessions; requires `--yes` or interactive confirmation), `remove` (comments out all `SERVAGENT_OAUTH_*` vars in `.env`, deletes OAuth database unless `--keep-db`; requires `--yes` or interactive confirmation). All `apikey` and `oauth` commands use `_find_env_file()` to auto-detect `.env` or accept `--env-file`. If the `.env` file is not writable (e.g. `/opt/servagent/.env` owned by root), the command automatically re-executes itself with `sudo` via `_reexec_with_sudo()`. Commands decorated with `@click.confirmation_option` pass `inject_yes=True` so that the re-executed process skips the already-answered prompt (stdin may be EOF). Helper functions `_env_set`, `_env_comment_out`, `_env_get` manipulate `.env` via regex
- **Issuer URL auto-detection**: `_guess_issuer_url()` resolves the OAuth issuer URL with three priorities: (1) `SERVAGENT_BASE_URL` — explicit public URL persisted by `install.sh` (source of truth), (2) `SERVAGENT_TLS_CERTFILE` — extract domain from Let's Encrypt path (standalone HTTPS), (3) fall back to the server's first public IP (HTTP-only / dev). The MCP SDK requires HTTPS for non-localhost issuer URLs (RFC 8414)
- **Base URL**: `SERVAGENT_BASE_URL` is written to `.env` by `install.sh` when a domain is provided (e.g. `https://servagent.example.com`). Format: `scheme://host[:port]` — port is omitted when standard (443/HTTPS, 80/HTTP). This is the authoritative source for the server's public URL, used by `servagent oauth setup` to derive the issuer URL
- **Startup validation**: `main()` validates that `OAUTH_ISSUER_URL` uses HTTPS for non-localhost hosts before the SDK can throw a confusing `ValueError`, providing a clear error message with fix instructions
- **File writing best practices** (in `_SERVER_RULES`): Instructions explicitly guide AI agents to use `write_file` for creating/overwriting files instead of `execute_command` with heredocs. `edit_file` is recommended for targeted modifications. `execute_command` should only be used for running programs (installs, services, git, docker). For multi-file projects, the rules prescribe a structured workflow: create dirs → write each file individually → install deps → verify
