...
- **Issuer URL auto-detection**: `_guess_issuer_url()` resolves the OAuth issuer URL in three steps: (1) extract domain from `SERVAGENT_TLS_CERTFILE` Let's Encrypt path, (2) if behind reverse proxy (`host=127.0.0.1`) with no TLS vars, scan `/etc/letsencrypt/live/` directory for domain names, (3) fall back to the server's first public IP. Scheme is `https` when: TLS is configured, a Let's Encrypt domain was found, or behind proxy with a domain name. The MCP SDK requires HTTPS for non-localhost issuer URLs (RFC 8414)
